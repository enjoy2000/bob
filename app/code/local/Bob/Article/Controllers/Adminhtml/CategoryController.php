...
class Bob_Article_Adminhtml_CategoryController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function indexAction() {
        $this->_initAction(); 
        $this->_addContent($this->getLayout()->createBlock('article/adminhtml_category'));
        $this->renderLayout();
    }
    ...
    public function editAction()
    {
        ...
        $articleModel  = Mage::getModel('article/category')->load($articleId);
 
        if ($articleModel->getId() || $articleId == 0) {
 
            Mage::register('category_data', $articleModel);
            ...
            $this->_addContent($this->getLayout()->createBlock('article/adminhtml_category_edit'))
                 ->_addLeft($this->getLayout()->createBlock('article/adminhtml_category_edit_tabs'));
    ...
    public function saveAction()
    {
        if ( $this->getRequest()->getPost() ) {
            try {
                ...
                $articleModel = Mage::getModel('article/category');
               
                $articleModel->setData($postData)
